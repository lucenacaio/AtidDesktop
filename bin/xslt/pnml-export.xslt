...
<xsl:template name="label">
    <xsl:param name="compositeActivitylabel"/>
    <xsl:choose>
        <xsl:when test="not(string(label))">
            <value>&#160;</value> <!-- empty label -->
        </xsl:when>
        <xsl:when test="not(string($compositeActivitylabel))">
            <value><xsl:value-of select="label"/></value>
        </xsl:when>
        <xsl:otherwise>
            <value><xsl:value-of select="concat($compositeActivitylabel,'.',label)"/></value>
        </xsl:otherwise>
    </xsl:choose>
</xsl:template>
...
<xsl:template name="compositeActivity">
    ...
    <xsl:for-each select="simpleActivity">
        <simpleActivity provider="petrinet.provider.simpleActivity">
            <xsl:attribute name="id"><xsl:value-of select="id"/></xsl:attribute>
            <name>
                <xsl:call-template name="label">
                    <xsl:with-param name="compositeActivitylabel"><xsl:value-of select="$label"/></xsl:with-param>
                </xsl:call-template>
                <graphics>
                    <offset x="5" y="33"/>
