...
import org.atid.petrinet.xml.DocumentExporter;
import org.atid.petrinet.xml.DocumentImporter;
...
public class PflowxFileType extends FileType {
    ...
    public String getName() {
        return "ATID Composite Activitys";
    }
    ...
    @Override
    public Icon getIcon() {
        final Icon icon = GraphicsTools.getIcon("atid/filechooser/pflowx.gif");
        return icon;
    }
    ...
    public Document load(File file) throws FileTypeException {
        try {
            ...
            document.petriNet.getRootCompositeActivity().setViewTranslationToCenterRecursively();
            return document;
        } catch (JAXBException ex) {
            if (!file.exists()) {
                throw new FileTypeException("File not found.");
