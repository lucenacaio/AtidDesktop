...
public class LocalClipboard {
    ...
    private CompositeActivity compositeActivity = new CompositeActivity();

    public LocalClipboard() {
    }

    public void setContents(Set<Element> elements, PetriNet petriNet) {
        compositeActivity.removeElements();
    ...
    public Set<Element> getContents(PetriNet petriNet) {
        return ElementCloner.getClones(compositeActivity.getElements(), petriNet);
    }

    public boolean isEmpty() {
        return compositeActivity.getElements().isEmpty();
    }
    ...
    private static Set<Element> filterOutDisconnectedArcs(Set<Element> elements) {
        ...
        for (Node node : nodes) {
            ...
            for (ArcEdge connectedArcEdge : connectedArcEdges) {
                if (nodes.contains(connectedArcEdge.getSimpleActivityNode()) && nodes.contains(connectedArcEdge.getTransitionNode())) {
