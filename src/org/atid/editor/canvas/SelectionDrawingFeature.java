...
class SelectionDrawingFeature implements Feature {
    ...
    public void mousePressed(MouseEvent event) {
        ...
        if (mouseButton == MouseEvent.BUTTON1
            ...
            if (event.isShiftDown()) {
                previousSelection.addAll(Atid.getRoot().getSelection().getElements());
            } else {
    ...
    public void setHoverEffects(int x, int y) {
        ...
        if (selecting) {
            Atid.getRoot().getSelection().clear();
            Atid.getRoot().getSelection().addAll(previousSelection);
            for (Element visualElement : Atid.getRoot().getDocument().petriNet.getCurrentCompositeActivity().getElements()) {
    ...
    private void addElementToSelection(Element element) {
        ...
        Atid.getRoot().getSelection().add(element);
    }
