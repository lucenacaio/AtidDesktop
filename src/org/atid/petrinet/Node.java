...
/**
 * Transition, SimpleActivity, ReferenceSimpleActivity, Subnet are subclasses of Node
 *
...
public abstract class Node extends Element implements Comparable<Node> {
    ...
    public Set<ArcEdge> getConnectedArcEdges() {
        ...
        for (ArcEdge arcEdge : getParentCompositeActivity().getArcEdges()) {
    ...
    public Set<Arc> getConnectedArcs() {
        ...
        for (Arc arc : getParentCompositeActivity().getArcs()) {
    ...
    public Set<Arc> getConnectedArcs(boolean simpleActivityToTransition) {
        Set<Arc> connectedArcs = new HashSet<Arc>();
        for (Arc arc : getParentSimpleActivity().getArcs()) {
            if ((arc.getSource() == this || arc.getDestination() == this) && arc.isSimpleActivityToTransition() == simpleActivityToTransition) {
                connectedArcs.add(arc);
            }
        }
        return connectedArcs;
    ...
    public Set<ReferenceArc> getConnectedReferenceArcs() {
        ...
        for (ReferenceArc referenceArc : getParentCompositeActivity().getReferenceArcs()) {
    ...
    public String getFullLabel() {
        ...
        CompositeActivity compositeActivity = getParentCompositeActivity();
        while (compositeActivity != null && compositeActivity.getParentCompositeActivity() != null) {
            fullLabel.insert(0, compositeActivity.getLabel() + ".");
            compositeActivity = compositeActivity.getParentCompositeActivity();
        }
...
//		GraphicsTools.drawString(g, getId(), getCenter().x, getStart().y, HorizontalAlignment.center, VerticalAlignment.bottom);
    ...
    public Node getClone() {
        ...
        Atid.getRoot().getDocument().getPetriNet().getNodeSimpleIdGenerator().setUniqueId(node);
        return node;
    }
}
