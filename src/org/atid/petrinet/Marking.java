...
public class Marking {
    ...
    protected Map<SimpleActivityNode, Integer> map = new ConcurrentHashMap<SimpleActivityNode, Integer>();
    ...
    public Marking(Marking marking) {
        ...
        try {
            this.map = new ConcurrentHashMap<SimpleActivityNode, Integer>(marking.map);
        } finally {
    ...
    private Set<Transition> getTransitions() {
        return petriNet.getRootCompositeActivity().getTransitionsRecursively();
    }
    ...
    /**
     * Returns the number of tokens based on the specified SimpleActivityNode (SimpleActivity or
 ReferenceSimpleActivity). If specified SimpleActivityNode is ReferenceSimpleActivity, it will return
 number of tokens of its connected SimpleActivity. If the specified ReferenceSimpleActivity
 is not connected to any SimpleActivity, it will return zero. If the resulting
 SimpleActivity is static, number of tokens will be given from initial marking
 instead.
     */
    public int getTokens(SimpleActivityNode simpleActivityNode) {
        SimpleActivityNode simpleActivity = simpleActivityNode.getSimpleActivity();
        if (simpleActivity == null) { // In case of disconnected ReferenceSimpleActivity, we want it to appear with zero tokens. Disconnected ReferenceSimpleActivitys can be found in stored subnets.
            return 0;
        }
        ...
        if (marking.map.get(simpleActivity) == null) { // SimpleActivity has zero tokens in the beginning. Not every simpleActivity is in map. Only those previously edited.
            return 0;
        }

        return marking.map.get(simpleActivity);
    }

    /**
     * Sets the number of tokens to the specified SimpleActivityNode (SimpleActivity or
 ReferenceSimpleActivity). If specified SimpleActivityNode is ReferenceSimpleActivity, it will set
 number of tokens to its connected SimpleActivity. If the specified ReferenceSimpleActivity
 is not connected to any SimpleActivity, it will throw RuntimeException. If the
 specified number of tokens is negative, it will throw RuntimeException.
 If the resulting SimpleActivity is static, number of tokens will be set to initial
 marking instead.
     */
    public void setTokens(SimpleActivityNode simpleActivityNode, int tokens) {
        if (tokens < 0) {
        ...
        if (simpleActivity == null) {
            //throw new RuntimeException("setTokens() to disconnected ReferenceSimpleActivity");
            throw new IllegalStateException("setTokens() to disconnected ReferenceSimpleActivity");
        }

        if (simpleActivityNode.isStatic()) {
            petriNet.getInitialMarking().map.put(simpleActivityNode, tokens);
        } else {
            this.map.put(simpleActivity, tokens);
        }
    }
                ...
                if (arc.isSimpleActivityToTransition()) {
                    if (arc.getType().equals(Arc.RESET)) {//reset arc is always fireable
                        ...
                        if (!arc.getType().equals(Arc.INHIBITOR)) {
                            if (getTokens(arc.getSimpleActivityNode()) < arc.getMultiplicity()) {  //normal arc
                                isEnabled = false;
                                break;
                            }
                        } else {
                            if (getTokens(arc.getSimpleActivityNode()) >= arc.getMultiplicity()) {//inhibitory arc
                                isEnabled = false;
                                break;
                            }
                        }
    ...
    public boolean equals(Object obj) {
        ...
        Set<SimpleActivityNode> simpleActivitys = new HashSet<SimpleActivityNode>(); // because map is sparse
        simpleActivitys.addAll(this.map.keySet());
        simpleActivitys.addAll(other.map.keySet());
        for (SimpleActivityNode simpleActivity : simpleActivitys) {
            if (this.getTokens(simpleActivity) != other.getTokens(simpleActivity)) {
                return false;
            }
        }
        return true;
    ...
    public int hashCode() {
        ...
        for (SimpleActivityNode simpleActivity : this.map.keySet()) { // because map is sparse
            hash = 73 * hash + this.getTokens(simpleActivity);
        }
        return hash;
    }
